                ...
                if last_seen_str:
                    try:
                        ...
                        if age.total_seconds() > 120:
                            f.write("⚠️  STATUS: Should be OFFLINE (threshold: 2 minutes)\n")
                        else:
                            f.write("✅ STATUS: Should be ONLINE (within 2 minute threshold)\n")
                    except Exception as e:
                        f.write(f"Error parsing lastSeen: {e}\n")
            
            f.write(f"\n{'='*80}\n")
            f.write(f"\nCurrent Time: {now.isoformat()}\n")
            f.write("Offline Threshold: Last seen > 2 minutes ago\n")
