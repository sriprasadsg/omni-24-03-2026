...
class ShadowAICapability(BaseCapability):
    ...
    def _resolve_ip(self, ip: str) -> str:
        """
        Resolve IP to hostname with a short timeout to prevent blocking.
        """
        if ip in self.dns_cache:
            return self.dns_cache[ip]
            
        try:
            # Set a default timeout for socket operations if not already set
            # Note: socket.setdefaulttimeout affects all sockets, so we use a more localized approach if possible
            # But for gethostbyaddr, we just handle the potential delay.
