...
- [x] 73-02-PLAN.md — request-scoped webhook events: check-in, consumable low stock, request approved/denied (wave 2)
- [x] 73-04-PLAN.md — ITAM ticketing-bridge adapter, ticket-creation orchestrator, and the manual create-ticket endpoint (wave 2)
- [x] 73-03-PLAN.md — background webhook sweeps: warranty expiring and licence expiring, tenant-bracketed (wave 3)
- [x] 73-06-PLAN.md — frontend: webhook event picker entries and the Create Ticket row action on both ITAM tables (wave 3)
- [x] 73-05-PLAN.md — audit-overdue sweep and both automatic ticket triggers, registered at startup (wave 4)

**Cross-cutting constraints:**

- The four non-ITAM routers that share the `_require_itam_admin` guard by import (LDAP, SAML/SSO, user management, API-key management) stay session-auth-only — an API key must never reach them.
- Every webhook dispatched from a background sweep is individually tenant-bracketed; an unbracketed dispatch fails silently and delivers to nobody.
- No condition may create more than one ticket for the same asset or request.
