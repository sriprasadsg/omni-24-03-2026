...
affects: [64-03-ldap, 64-04-sso, 65-core-data-audit-customization]
...
# Phase 64 Plan 02: RBAC extension with ITAM roles, fixed normalization, super-admin guard Summary
...
- `get_permissions_for_role()` is the read-side contract `RoleEditorModal.tsx` needs; wiring it into an endpoint (and building `saveRole()`'s real persistence, still a mock per 64-01) remains explicitly deferred to a future plan, per 64-01's `<frontend_scope>` reasoning (`services/apiService.ts` is owned by Phase 65).
