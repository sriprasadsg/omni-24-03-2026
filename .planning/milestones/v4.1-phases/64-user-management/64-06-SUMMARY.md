...
# Phase 64 Plan 06: 2FA/TOTP — fix MFA pitfalls, password-confirmed disable/regenerate Summary
...
- New `POST /api/mfa/backup-codes/regenerate` endpoint (`{password}` -> new `backup_codes[]`) — backend-only, no frontend consumer yet per the plan's `<frontend_scope>` (deferred to the Phase 65 ITAM console).
...
- The new `POST /api/mfa/backup-codes/regenerate` route has no frontend consumer yet — deferred to Phase 65's ITAM console work per this plan's `<frontend_scope>`, which owns the frontend service layer additions going forward.
