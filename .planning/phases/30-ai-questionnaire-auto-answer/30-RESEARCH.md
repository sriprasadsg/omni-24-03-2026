...
1. **How does an inbound questionnaire actually enter the system — manual entry, or file upload/parsing?**
   ...
   - Recommendation: **Build manual question-entry only for this phase** — a form/endpoint mirroring `QuestionnaireCreate`'s existing `questions: List[Dict]` shape, with an optional client-side convenience (paste newline-delimited text, split into rows) but no server-side file parsing. This is the lightest-weight interpretation consistent with CLAUDE.md's "do what has been asked; nothing more," and RAG-01/RAG-02's actual emphasis (grounded drafting + mandatory review) is about what happens *after* a question exists, not how it arrives. If real file-upload/parsing (BULK-01-style zip+manifest, or a CSV/XLSX column-mapper) is truly required, it should be scoped as an explicit follow-up phase, not silently absorbed into this one's estimate.
   - **This must be confirmed before or during planning** — it materially changes phase scope/sizing (a manual-entry form is a small addition; a robust multi-format questionnaire parser is comparable in size to Phase 24's IaC scanner).

2. **Does the existing ChromaDB collection (`omni-agent-knowledge`) contain any pre-existing ingested content that needs tenant re-attribution, or can tenant-scoping start fresh?**
   ...
   - Recommendation: Inspect `ingest_security_standards.py`'s actual ingested content before implementing the tenant-scoping fix — if it's generic framework reference text (not tenant-specific evidence), the fix may need a `tenantId: "global"`-style sentinel value that's included in every tenant's `where` filter (via `$or`), rather than a strict per-tenant partition. Flag this as a required investigation step in Wave 0 of the plan, not an assumption.
