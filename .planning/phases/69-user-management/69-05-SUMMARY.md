...
# Phase 69 Plan 05: API token management — lifecycle, scopes, expiration, rate limits Summary
...
- The new scope/expiry/rate-limit fields are deliberately NOT surfaced in `SettingsApiKeysTab.tsx` (per this plan's `<frontend_scope>` — that UI work is deferred to Phase 70's ITAM console, which owns `services/apiService.ts`). The backend contract is additive so that phase can consume it without a migration.
