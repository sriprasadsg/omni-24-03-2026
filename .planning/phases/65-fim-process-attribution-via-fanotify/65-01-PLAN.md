...
must_haves:
  ...
  artifacts:
    - src/fim_fanotify_watcher.rs (Rust module for fanotify interaction)
    - src/fim_process_mapper.rs (Rust module for basic PID lookup)
    - tests/fim_fanotify_test.rs (Unit test for fanotify watcher)
  key_links:
    - Proper initialization of the fanotify API.
    - Correct extraction of event metadata, specifically PID.
    - Basic read of /proc filesystem for process name.
---
...
<task type="tracer">
  ...
  <action>
    ...
    5. Ensure `fanotify` crate is added to `Cargo.toml`.
  </action>
  <verify>
...
<task type="auto">
  ...
  <action>
    Enhance `src/fim_fanotify_watcher.rs`:
  ...
  <done>
    The `fim_fanotify_watcher.rs` module can correctly parse multiple fanotify event types into a structured `FanotifyEvent` enum/struct. Existing tests pass.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    Enhance `src/fim_process_mapper.rs`:
  ...
  <done>
    The `fim_process_mapper.rs` module can successfully build a basic parent-child process tree for a given PID, returning a list of `ProcessInfo` structs. Existing tests pass.
  </done>
</task>
...
<verification>
The `fim_fanotify_test` integration test successfully captures a file modification event, extracts the PID, and traces its parent process chain, verifying the core fanotify and basic process mapping functionality.
</verification>

<success_criteria>
- The `fim_fanotify_watcher.rs` module compiles successfully.
- The `fim_process_mapper.rs` module compiles successfully.
- The `cargo test --test fim_fanotify_test` command passes without errors.
- The output of the test demonstrates a successful fanotify event capture and retrieval of process name and parent PIDs for a test process.
</success_criteria>
