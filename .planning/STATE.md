...
milestone: v4.1
current_phase: 74
current_phase_name: Interactive Remote Desktop Control
status: idle
stopped_at: Phase 74 complete — all 5 plans committed, ROADMAP/STATE updated
...
**Current focus:** Phase 74 — Interactive Remote Desktop Control
...
- [Phase ?]: IT-0001 zero-padded counter, six-value verbatim vocabulary, read-time default (RECOMMENDED). Asset tags will be IT-{seq:04d}, lifecycle status will be one of the six verbatim ROADMAP values, and pre-existing agent assets will read as 'deployable' via a read-time default.
- [Phase ?]: 56-02: CATALOG_MODELS per-kind request-body registry lets one generic route validate five different Pydantic body shapes without five route functions
- [Phase ?]: 56-02: fieldsets embedded on the asset Model document, validated by router-independent itam_catalog_service.py so 56-04's asset write path can import validation without importing a router
- [Phase ?]: 57-01: assignedToType/assignedToId polymorphic pair (PD-01) rather than reusing locationId for location checkouts
- [Phase ?]: 57-01: checkout guard folded into find_one_and_update filter (deployable-or-absent), never a preceding conditional read (PD-03/atomicity)
- [Phase ?]: 57-01: repaired Phase-56 defect - removed erroneous await on synchronous cache_service.invalidate_cache in create_manual_asset
- [Phase ?]: 57-01: split test_itam_lifecycle.py (609 lines) into itam_lifecycle_test_support.py + test_itam_lifecycle.py + test_itam_lifecycle_expansion.py to satisfy CLAUDE.md's 500-line limit
...
- [Phase ?]: Split each task's shared-file edits into two atomic commits by building the full implementation then temporarily reverting Task-2-only hunks for a clean Task-1 commit before reapplying
- [Phase ?]: license_utilization's Manufacturer column shows the raw manufacturerId, not a resolved name — no batched manufacturers lookup was specified by the plan
- [Phase ?]: Custom report component join keys off components.parentAssetId (the field the real attach/detach service writes), not ComponentCreate.assetIds — the plan's literal wording named a request-model field the service never persists.
- [Phase ?]: run_custom_report re-verifies every filter condition in Python against the fully resolved post-join row (shared _condition_passes helper), so the Mongo-query optimization and the correctness guarantee cannot diverge and remain testable without a real MongoDB.
- [Phase ?]: 72-04: compute_itam_kpis fans one batched db.assets.find() out to assetValue/warrantyExpirations/overdue; assetCount/statusBreakdown cover every tenant asset (not just those with a purchase record) so hasData reflects fleet-wide zero-asset state
- [Phase ?]: 72-04: GET /api/itam/kpis mounted at a distinct prefix (never a sub-path of /api/itam/reports), gated by the real _require_itam_admin import, registered directly after itam_reporting_endpoints in router_registry.py
...
- [Phase ?]: Plan 73-05: STUCK_APPROVAL_TICKET_EVENT_TYPE is a local string constant (asset_request.stuck_approval), not a new EVENT_* webhook constant — D-05 defines no webhook event for a stuck approval; it only feeds ticketing_bridge's mechanical type derivation.
- [Phase ?]: Plan 73-05: fixed constants for the previously-unspecified thresholds — STUCK_APPROVAL_WINDOW_DAYS=7, HIGH_VALUE_REQUEST_COST=2000, HIGH_VALUE_REQUEST_QUANTITY=10, ITAM_EVENT_SWEEP_INTERVAL_SECONDS=86400 (daily, not the warranty job's hourly cadence).
...
|------|----------|-------|-------|
| Phase 52-file-integrity-monitoring P01 | 2m 14s | 2 tasks | 2 files |
...
| Phase 72 P02 | 45min | 2 tasks | 3 files |
| Phase 72 P03 | 30min | 2 tasks | 5 files |
| Phase 72 P04 | 46min | 2 tasks | 5 files |
...
| Phase 73 P05 | 55min | 3 tasks | 6 files |
...
<!-- Phase 64 P01-P06 rows removed 2026-08-13: bogus data (duration 0, 1 task, 1 file each) —
     ...
     work. See .planning/phases/69-user-management/ for the real SUMMARY.md files. -->

## Last Session

- **Timestamp:** 2026-08-25T00:46:22Z
- **Stopped at:** context exhaustion at 100% (2026-08-28)
...
**Last session:** 2026-08-28T08:38:25.824Z
...
Phase: 74 (Interactive Remote Desktop Control) — EXECUTING
Plan: 1 of 5
Status: Executing Phase 74
Last activity: 2026-08-29 — Phase 74 execution resumed (wave continue)

## Deferred Items

- **[2026-08-13] Roadmap track collision (64-68 vs. root's own 64-66):** Root `.planning/ROADMAP.md` and `.planning/milestones/v4.1-ROADMAP.md` independently claimed phase numbers 64-68 for unrelated work (this is what caused the stray `milestone: v4.1` frontmatter and the fake `Phase 64 P01-P06` metrics rows removed from this file today). Fixed by moving the two v4.1 phases with real content (User Management, Core Data/Audit/Customization) into root `ROADMAP.md`/`.planning/phases/` as 69/70, and forward-declaring 71-73 for the not-yet-planned Procurement/Reporting/API phases. `current_phase_name`/`status`/`stopped_at` in this file's frontmatter still reflect an older, unrelated AI-narrative-migration stopping point (Phase 39) — not touched by this fix, a separate pre-existing staleness issue (see the "Current Position staleness" deferred item below for the established pattern of this recurring).
- **[2026-08-17] Phase 64 rotate_key: second, separate track collision (distinct from the 2026-08-13 note above):** Two independent phase-64 planning attempts existed simultaneously in `.planning/phases/` — `64-rotate-key-autonomous-remediation-action/` (hyphen, canonical, full discuss→research→plan→verify pipeline, targets the shipped `agent-install/omni-agent-rs/` tree) and `64-rotate_key-autonomous-remediation-action/` (underscore, targets the legacy unshipped `agent-rust/` tree with self-documented stubbed rotation logic). ROADMAP.md's Phase 64 checkboxes described the canonical track's plans but had absorbed a mismatched summary from the underscore track as false completion evidence for plan 03 — the canonical track's real rotation-mechanics plan (byte-for-byte backup/restore, grounded re-verify) had never actually been executed, and its own plan text defers dispatch-arm wiring to a plan 64-04 that didn't exist. Fixed: underscore directory moved to `.planning/phases/_superseded-64-vault-legacy-rust-track/` (with SUPERSEDED.md explaining what real, still-live code it produced — Vault client, `/rotate-key` AI chat skill — which was left untouched, just no longer counted as phase-64 evidence); ROADMAP.md corrected to 2/4 plans complete; plan 03 execution and plan 04 (dispatch-arm) planning+execution now in progress.
- **[2026-08-17, later] Phase 64 rotate_key: plan 04 executed — canonical track now 4/4 plans complete.** `agent-install/omni-agent-rs/src/instructions.rs` gained `"rotate_key"`/`"rotate_key_rollback"` dispatch arms wired to new `remediation_actions::rotate_key`/`rotate_key_rollback` wrappers over plan 64-03's `ssh_key_rotation::rotate_in_place`/`rollback_rotation`. `backend/playbooks/rotate_key.yaml`'s rollback step — previously `action: restore_file` with an unresolvable `finding.details.rotate_key_backup_path` reference — corrected to `action: rotate_key_rollback`/`authorized_keys_path`; the pre-existing `test_rotate_key_wiring.py` was actually failing on this exact gap before the fix (`KeyError: 'authorized_keys_path'`), contradicting 64-01-SUMMARY.md's "all 5 tests passed" claim. 5 new Rust integration tests (`tests/remediation_dispatch_test.rs`) drive the real dispatcher against fixture files; `cargo test rotate_key` 34/34, `cargo test --lib` 83/83, full `cargo test` shows only the 2 pre-existing documented failure classes (stale capability-count assertions, `naughtyfy` Windows cross-compile — see `deferred-items.md`). This note intentionally does NOT touch this file's frontmatter `current_phase`/`stopped_at` (tracking the separate, active Phase 72 milestone track) — ROADMAP.md's Phase 64 section updated directly instead. This executor run deliberately did not invoke `gsd-tools query state.advance-plan`/`roadmap.update-plan-progress` given the documented phase-64 collision risk — STATE.md/ROADMAP.md Phase-64 sections were updated by hand instead.
- **[2026-08-17, later still] Phase 64 rotate_key: plan 02 re-executed for real — canonical track now genuinely 4/4 plans complete.** Task 1 (`ssh_key_checks.rs` weak-key predicate/parser, reopened after re-verification found its 8 required tests and module doc comment missing) had already been fixed in a prior commit (d984fc5b) this session. Task 2 (`check_authorized_keys()`/`weak_key_finding()` wired into `scan_misconfigurations()`, test module split to `vulnerability_scan_tests.rs` per the 500-line convention) was found already implemented as uncommitted working-tree changes — verified against every plan acceptance criterion (`cargo test`: 8 ssh_key_checks + 16 vulnerability_scan tests green; 0 unused-warnings in either new file; `ssh-key` pinned 0.6.7 no `-rc`) and committed (3a930718). The Windows cross-compile acceptance check and 7 `cargo test` integration failures were verified pre-existing and unrelated by stashing this plan's diff and reproducing identically (phase 65's `naughtyfy` crate breaks `x86_64-pc-windows-gnu`; `tests/integration.rs` has a stale hardcoded capability count and 2 schema-drift asserts) — both already tracked in `deferred-items.md`, not newly introduced. `64-02-SUMMARY.md` written (b31c4219). Same manual-STATE/ROADMAP-update posture as the plan-04 note above, for the same collision-risk reason.
