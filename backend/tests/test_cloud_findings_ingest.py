"""Phase 32 tests — M365 / MongoDB Atlas findings ingest and scan_account dispatch.

scan_account does function-local imports of the flat modules
(m365_ingest, mongodb_atlas_ingest, cloud_checks_service), so patch
targets must be the flat module attributes — they are resolved at call
...
def mock_db():
    ...
    db.cloud_accounts.find_one = AsyncMock(return_value=None)
...
async def test_m365_poll_success():
    ...
    with patch("m365_ingest.ConfidentialClientApplication", return_value=mock_app), \
...
async def test_scan_account_m365_dispatch(mock_db):
    ...
    with patch("cloud_accounts_service._decrypt", return_value=config_json), \
        ...
        assert mock_ingest.call_args.args[0] == {"tenant_id": "common", "client_id": "c", "client_secret": "s"}
...
async def test_scan_account_aws_no_ingest_unchanged(mock_db):
    ...
    with patch("cloud_accounts_service._decrypt", return_value="{}"), \
