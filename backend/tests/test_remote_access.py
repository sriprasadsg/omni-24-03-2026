...
Comprehensive tests for the remote access subsystem:
  ...
  - agent_remote_control.py — execute / restart / status / security guards
"""
import sys
import os
...
# ─────────────────────────────────────────────────────────────────────────────
# 3.  Agent Remote Control  (agent_remote_control.py)
# ─────────────────────────────────────────────────────────────────────────────

class TestAgentRemoteControl:

    @pytest.fixture(autouse=True)
    def setup(self):
        import agent_remote_control as mod
        self.mod = mod
        self.user = _user(role="super_admin")
        self.db = _db()
        # Simulate an online agent belonging to the test tenant
        self.db.agents.find_one = AsyncMock(
            return_value={"id": "agent-1", "tenantId": "tenant-1", "hostname": "host-1"}
        )
        self.app = _app(mod.router, self.user)

    def _wm(self, connected=True, send_ok=True):
        wm = MagicMock()
        wm.is_agent_connected = AsyncMock(return_value=connected)
        wm.send_to_agent = AsyncMock(return_value=send_ok)
        return wm

    # ── execute command ───────────────────────────────────────────────────────

    def test_execute_allowed_command_returns_success(self):
    ...
    def test_execute_blocked_command_returns_400(self):
        """'rm' is not in the allowlist — must be rejected."""
        with patch("agent_remote_control.websocket_manager", self._wm()):
            with patch("agent_remote_control.get_database", return_value=self.db):
                with TestClient(self.app) as c:
                    r = c.post("/api/agents/remote/agent-1/execute",
    ...
    def test_execute_shell_metachar_semicolon_returns_400(self):
        """Semicolon in args is a shell injection vector — must be rejected."""
        with patch("agent_remote_control.websocket_manager", self._wm()):
            with patch("agent_remote_control.get_database", return_value=self.db):
                with TestClient(self.app) as c:
                    r = c.post("/api/agents/remote/agent-1/execute",
                               json={"command": "ls", "args": ["; rm -rf /"]})
        assert r.status_code == 400
        assert "metacharacter" in r.json()["detail"].lower()

    def test_execute_shell_metachar_pipe_returns_400(self):
        with patch("agent_remote_control.websocket_manager", self._wm()):
            with patch("agent_remote_control.get_database", return_value=self.db):
                with TestClient(self.app) as c:
                    r = c.post("/api/agents/remote/agent-1/execute",
                               json={"command": "ls", "args": ["| cat /etc/passwd"]})
        assert r.status_code == 400

    def test_execute_shell_metachar_backtick_returns_400(self):
        with patch("agent_remote_control.websocket_manager", self._wm()):
            with patch("agent_remote_control.get_database", return_value=self.db):
                with TestClient(self.app) as c:
                    r = c.post("/api/agents/remote/agent-1/execute",
                               json={"command": "ls", "args": ["`id`"]})
        assert r.status_code == 400

    def test_execute_agent_not_connected_returns_503(self):
        with patch("agent_remote_control.websocket_manager", self._wm(connected=False)):
            with patch("agent_remote_control.get_database", return_value=self.db):
                with TestClient(self.app) as c:
                    r = c.post("/api/agents/remote/agent-1/execute",
                               json={"command": "ls", "args": []})
    ...
    def test_execute_agent_not_found_returns_404(self):
        self.db.agents.find_one = AsyncMock(return_value=None)
        with patch("agent_remote_control.websocket_manager", self._wm()):
            with patch("agent_remote_control.get_database", return_value=self.db):
                with TestClient(self.app) as c:
                    r = c.post("/api/agents/remote/agent-1/execute",
    ...
    def test_execute_logs_to_agent_commands(self):
        """Every executed command must be recorded in agent_commands collection."""
        with patch("agent_remote_control.websocket_manager", self._wm()):
            with patch("agent_remote_control.get_database", return_value=self.db):
                with TestClient(self.app) as c:
                    c.post("/api/agents/remote/agent-1/execute",
                           json={"command": "ps", "args": []})

        self.db.agent_commands.insert_one.assert_called_once()
        logged = self.db.agent_commands.insert_one.call_args[0][0]
        assert logged["command"] == "ps"
    ...
    def test_restart_agent_offline_returns_503(self):
        with patch("agent_remote_control.websocket_manager", self._wm(connected=False)):
            with patch("agent_remote_control.get_database", return_value=self.db):
                with TestClient(self.app) as c:
                    r = c.post("/api/agents/remote/agent-1/restart")
    ...
    def test_status_online_agent(self):
        with patch("agent_remote_control.websocket_manager", self._wm(connected=True)):
            with TestClient(self.app) as c:
                r = c.get("/api/agents/remote/agent-1/status")
        assert r.status_code == 200
        assert r.json()["websocket_connected"] is True
        assert r.json()["agent_id"] == "agent-1"

    def test_status_offline_agent(self):
        with patch("agent_remote_control.websocket_manager", self._wm(connected=False)):
            with TestClient(self.app) as c:
                r = c.get("/api/agents/remote/agent-1/status")
        assert r.status_code == 200
        assert r.json()["websocket_connected"] is False

    # ── allowlist coverage ────────────────────────────────────────────────────

    @pytest.mark.parametrize("cmd", [
        "ls", "ps", "whoami", "hostname", "df", "uptime", "date",
        "ifconfig", "ip", "id", "uname", "pwd", "free",
    ])
    def test_allowlisted_commands_are_accepted(self, cmd):
        with patch("agent_remote_control.websocket_manager", self._wm()):
            with patch("agent_remote_control.get_database", return_value=self.db):
                with TestClient(self.app) as c:
                    r = c.post("/api/agents/remote/agent-1/execute",
                               json={"command": cmd, "args": []})
        assert r.status_code == 200, f"Expected {cmd!r} to be allowed, got {r.status_code}"

    @pytest.mark.parametrize("cmd", [
        "rm", "dd", "chmod", "wget", "curl", "bash", "sh", "python",
        "nc", "netcat", "sudo", "su", "reboot", "shutdown",
    ])
        ...
        with patch("agent_remote_control.websocket_manager", self._wm()):
            with patch("agent_remote_control.get_database", return_value=self.db):
                with TestClient(self.app) as c:
                    r = c.post("/api/agents/remote/agent-1/execute",
                               json={"command": cmd, "args": []})
        assert r.status_code == 400, f"Expected {cmd!r} to be blocked, got {r.status_code}"
