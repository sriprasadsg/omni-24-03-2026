...
# ── Real-time ETW telemetry (agent-rust ETW engine, Phase 1) ──────────────────
# Receives batched host events (process start/stop, later network/registry/file) from
# the Windows agent's ETW engine. See agent-rust/docs/etw-telemetry-design.md.
...
async def ingest_telemetry(
    ...
    return {"ok": True, "stored": len(docs), "batch_id": batch_id}
