...
IAC_CHECKS = [
    # ─── Terraform checks ──────────────────────────────────────────────────
    {"id": "tf-s3-public-acl", "name": "S3 Bucket Public ACL", "description": "S3 bucket should not have public ACL", "provider": "terraform", "severity": "critical", "pattern": r'resource\s+"aws_s3_bucket"\s+"([^"]+)"', "negative_pattern": r'acl\s*=\s*"public-read"|acl\s*=\s*"public-read-write"'},
    {"id": "tf-iam-wildcard", "name": "IAM Policy Wildcard Action", "description": "IAM policy should not use * action", "provider": "terraform", "severity": "critical", "pattern": r'resource\s+"aws_iam_role_policy"\s+"[^"]*"|resource\s+"aws_iam_policy"\s+"[^"]*"', "negative_pattern": r'Action\s*=\s*\[?\s*"\*"\s*\]?'},
    {"id": "tf-sg-open-ssh", "name": "Security Group Open SSH", "description": "Security group should not allow 0.0.0.0/0 on port 22", "provider": "terraform", "severity": "critical", "pattern": r'resource\s+"aws_security_group"\s+"[^"]*"', "negative_pattern": r'cidr_blocks\s*=\s*\[?\s*"0\.0\.0\.0/0".*from_port\s*=\s*22|from_port\s*=\s*22.*cidr_blocks\s*=\s*\[?\s*"0\.0\.0\.0/0"'},
    {"id": "tf-sg-open-rdp", "name": "Security Group Open RDP", "description": "Security group should not allow 0.0.0.0/0 on port 3389", "provider": "terraform", "severity": "critical", "pattern": r'resource\s+"aws_security_group"\s+"[^"]*"', "negative_pattern": r'cidr_blocks\s*=\s*\[?\s*"0\.0\.0\.0/0".*from_port\s*=\s*3389|from_port\s*=\s*3389.*cidr_blocks\s*=\s*\[?\s*"0\.0\.0\.0/0"'},
    {"id": "tf-ebs-encrypted", "name": "EBS Volume Encryption Disabled", "description": "EBS volume should have encryption enabled", "provider": "terraform", "severity": "high", "pattern": r'resource\s+"aws_ebs_volume"\s+"[^"]*"', "negative_pattern": r'encrypted\s*=\s*false'},
    {"id": "tf-rds-encrypted", "name": "RDS Storage Encryption Disabled", "description": "RDS instance should have storage encryption enabled", "provider": "terraform", "severity": "high", "pattern": r'resource\s+"aws_db_instance"\s+"[^"]*"', "negative_pattern": r'storage_encrypted\s*=\s*false'},
    {"id": "tf-eks-public", "name": "EKS Cluster Public Endpoint", "description": "EKS cluster endpoint should be private", "provider": "terraform", "severity": "high", "pattern": r'resource\s+"aws_eks_cluster"\s+"[^"]*"', "negative_pattern": r'endpoint_public_access\s*=\s*true'},
    {"id": "tf-ec2-public-ip", "name": "EC2 Instance with Public IP", "description": "EC2 instance should not have public IP", "provider": "terraform", "severity": "high", "pattern": r'resource\s+"aws_instance"\s+"[^"]*"', "negative_pattern": r'associate_public_ip_address\s*=\s*true'},
    {"id": "tf-alb-http", "name": "ALB HTTP Listener (no redirect)", "description": "ALB listener should redirect HTTP to HTTPS", "provider": "terraform", "severity": "high", "pattern": r'resource\s+"aws_alb_listener"\s+"[^"]*"|resource\s+"aws_lb_listener"\s+"[^"]*"', "negative_pattern": r'port\s*=\s*80'},
    {"id": "tf-kms-rotation", "name": "KMS Key Rotation Disabled", "description": "KMS key should have rotation enabled", "provider": "terraform", "severity": "medium", "pattern": r'resource\s+"aws_kms_key"\s+"[^"]*"', "negative_pattern": r'rotation_enabled\s*=\s*false'},
    ...
    {"id": "tf-ec2-admin", "name": "EC2 Admin Privileged True", "description": "EC2 instance should not run as admin", "provider": "terraform", "severity": "high", "pattern": r'resource\s+"aws_instance"\s+"[^"]*"', "negative_pattern": r'user_data.*admin|user_data.*root'},
    ...
    {"id": "tf-rds-deletion-protection", "name": "RDS Deletion Protection Disabled", "description": "RDS should have deletion protection", "provider": "terraform", "severity": "medium", "pattern": r'resource\s+"aws_db_instance"\s+"[^"]*"', "negative_pattern": r'deletion_protection\s*=\s*false'},
    # ─── Kubernetes checks ──────────────────────────────────────────────────
