...
IAC_CHECKS = [
    ...
    {"id": "cfn-s3-public-access", "name": "S3 Bucket Public Access Not Blocked", "description": "S3 bucket should have PublicAccessBlockConfiguration blocking public ACLs/policies", "provider": "cloudformation", "severity": "critical", "pattern": r'Type"?\s*:?\s*"?AWS::S3::Bucket', "negative_pattern": r'PublicAccessBlockConfiguration', "scope_lines": 30},
    ...
    {"id": "cfn-sg-open-ssh", "name": "Security Group Open SSH", "description": "SecurityGroupIngress should not allow 0.0.0.0/0 on port 22", "provider": "cloudformation", "severity": "critical", "pattern": r'Type"?\s*:?\s*"?AWS::EC2::SecurityGroup', "negative_pattern": r'CidrIp"?\s*:\s*"?0\.0\.0\.0/0.*(FromPort"?\s*:\s*22|ToPort"?\s*:\s*22)|(FromPort"?\s*:\s*22|ToPort"?\s*:\s*22).*CidrIp"?\s*:\s*"?0\.0\.0\.0/0', "vulnerable_marker": True, "scope_lines": 15},
    {"id": "cfn-sg-open-rdp", "name": "Security Group Open RDP", "description": "SecurityGroupIngress should not allow 0.0.0.0/0 on port 3389", "provider": "cloudformation", "severity": "critical", "pattern": r'Type"?\s*:?\s*"?AWS::EC2::SecurityGroup', "negative_pattern": r'CidrIp"?\s*:\s*"?0\.0\.0\.0/0.*(FromPort"?\s*:\s*3389|ToPort"?\s*:\s*3389)|(FromPort"?\s*:\s*3389|ToPort"?\s*:\s*3389).*CidrIp"?\s*:\s*"?0\.0\.0\.0/0', "vulnerable_marker": True, "scope_lines": 15},
    ...
    {"id": "cfn-eks-public-endpoint", "name": "EKS Cluster Public Endpoint", "description": "EndpointPublicAccess should be false (defaults to true if omitted)", "provider": "cloudformation", "severity": "high", "pattern": r'Type"?\s*:?\s*"?AWS::EKS::Cluster', "negative_pattern": r'EndpointPublicAccess"?\s*:\s*false', "scope_lines": 20},
    ...
    {"id": "cfn-s3-logging-disabled", "name": "S3 Bucket Logging Disabled", "description": "S3 bucket should configure LoggingConfiguration", "provider": "cloudformation", "severity": "medium", "pattern": r'Type"?\s*:?\s*"?AWS::S3::Bucket', "negative_pattern": r'LoggingConfiguration', "scope_lines": 30},
    ...
    {"id": "cfn-s3-versioning-disabled", "name": "S3 Versioning Disabled", "description": "S3 bucket should configure VersioningConfiguration Status: Enabled", "provider": "cloudformation", "severity": "medium", "pattern": r'Type"?\s*:?\s*"?AWS::S3::Bucket', "negative_pattern": r'VersioningConfiguration', "scope_lines": 30},
