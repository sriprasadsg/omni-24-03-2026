...
function Confirm-Checksum([string]$ApiUrl, [string]$RemoteName, [string]$LocalPath) {
    ...
    Write-Host "  Refusing to install a file that does not match the platform's checksum." -ForegroundColor Red
    return $false
}
...
Write-Step "Verifying binary checksum..."
if (-not (Confirm-Checksum -ApiUrl $ApiUrl -RemoteName "omni-agent.exe" -LocalPath $TmpExe)) {
