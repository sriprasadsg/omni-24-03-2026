...
@router.get("/scans")
async def list_pipeline_scans(db=Depends(_db)):
    """List recent pipeline security scan results."""
    cursor = db["pipeline_scans"].find({}, {"_id": 0}).sort("scanned_at", -1).limit(50)
    items = await cursor.to_list(length=50)
    return items


@router.get("/scans/{scan_id}")
async def get_scan(scan_id: str, db=Depends(_db)):
    doc = await db["pipeline_scans"].find_one({"id": scan_id}, {"_id": 0})
    if not doc:
...
@router.get("/policies")
async def list_policies(db=Depends(_db)):
    """Return pipeline security gate policies."""
    cursor = db["pipeline_policies"].find({}, {"_id": 0})
...
@router.get("/stats")
async def pipeline_stats(db=Depends(_db)):
    total = await db["pipeline_scans"].count_documents({})
    passed = await db["pipeline_scans"].count_documents({"status": "passed"})
    failed = await db["pipeline_scans"].count_documents({"status": "failed"})
    blocked = await db["pipeline_scans"].count_documents({"status": "blocked"})

    # Aggregate finding counts across all scan documents
