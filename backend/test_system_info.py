...
print("\n1. CPU DETECTION:")
try:
    result = subprocess.check_output(["wmic", "cpu", "get", "name"], shell=False, timeout=5).decode(errors='ignore')
...
print("\n3. SERIAL NUMBER:")
try:
    result = subprocess.check_output(["wmic", "bios", "get", "serialnumber"], shell=False, timeout=5).decode(errors='ignore')
    print(f"Raw output: {repr(result)}")
