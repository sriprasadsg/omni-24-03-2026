...
from cissp_oracle_data import CISSP_DOMAINS, CISSP_SYSTEM_PROMPT
...
async def _generate_cissp_response(message: str, context: str, findings: list, domain_filter: Optional[int]) -> dict:
    ...
    try:
        ...
        logger.debug("Platform LLM CISSP analysis failed, trying fallback: %s", e)
    ...
    try:
        ...
        logger.debug("Ollama CISSP analysis failed, using rule-based fallback: %s", e)
