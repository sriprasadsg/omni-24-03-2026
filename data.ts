...
const generateMetricData = (base: number, variance: number, points: number = 30) => {
    ...
    for (let i = 0; i < points; i++) {
        data.push({
            time: `${i}m ago`,
            value: Math.round(Math.min(100, Math.max(0, base + Math.sin(i * 0.7) * (variance / 2)))),
        });
    }
    return data;
};
...
export const AI_SYSTEMS_DATA: AiSystem[] = [
    {
        ...
        performanceData: Array.from({ length: 30 }, (_, i) => ({ time: `${30 - i}m`, latency: parseFloat((120 + Math.sin(i * 0.4) * 10).toFixed(1)), throughput: parseFloat((500 + Math.cos(i * 0.3) * 25).toFixed(1)), errorRate: parseFloat((1 + Math.abs(Math.sin(i * 0.6)) * 0.25).toFixed(2)) })),
        securityAlerts: []
    },
    {
        id: 'ai-sys-2',
