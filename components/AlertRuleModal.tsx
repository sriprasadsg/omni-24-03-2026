...
export const AlertRuleModal: React.FC<AlertRuleModalProps> = ({ isOpen, onClose, onSave, rule }) => {
  ...
  const inputCls = "mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md";
  ...
  const footer = (
    <>
      ...
      <button type="submit" form="alert-rule-form" className="px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700">Save Rule</button>
    </>
  );

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={rule ? 'Edit Alert Rule' : 'Create New Alert Rule'} size="lg" footer={footer}>
      <form id="alert-rule-form" onSubmit={handleSubmit}>
        <div className="space-y-4">
          <div>
            <label htmlFor="name" className={labelCls}>Rule Name</label>
            <input type="text" name="name" value={formData.name} onChange={handleChange} required className={inputCls} />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                  <label htmlFor="metric" className={labelCls}>Metric</label>
                  <select name="metric" value={formData.metric} onChange={handleChange} className={inputCls}>
               ...
               <div>
                  <label htmlFor="threshold" className={labelCls}>Threshold</label>
                  <input type="number" name="threshold" value={formData.threshold} onChange={handleChange} required className={inputCls} />
              </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                  <label htmlFor="duration" className={labelCls}>Duration (mins)</label>
