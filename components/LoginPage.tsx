...
import { BotIcon, MailIcon, KeyIcon, ShieldIcon, CpuIcon, FileTextIcon } from './icons';
...
const SmartCTA: React.FC<{ isLoading: boolean }> = ({ isLoading }) => {
    ...
    const label = lastView ? 'Welcome back' : 'Get started';
    const subtext = lastView ? `Continue exploring ${lastView}` : 'Deploy your first AI agent in minutes';
...
const HeroSection: React.FC = () => {
    const features = [
        { icon: <ShieldIcon size={24} className="text-primary-400" />, title: 'Zero Trust Security', description: 'Continuous agent-based monitoring and automated remediation across your entire fleet.' },
        { icon: <CpuIcon size={24} className="text-primary-400" />, title: 'Autonomous AI Agents', description: 'Self-healing endpoints with intelligent vulnerability detection and patch orchestration.' },
        { icon: <FileTextIcon size={24} className="text-primary-400" />, title: 'Compliance Automation', description: 'Map to 15+ frameworks including SOC2, HIPAA, ISO 27001 in real time.' },
    ];
    ...
    return (
        <aside className="hidden lg:flex flex-col items-center justify-center px-12 py-10 text-center relative z-10">
            <div className="mb-8">
                ...
                <p className="text-md text-slate-400 mt-2 font-medium tracking-wide uppercase">
            ...
            <div className="grid grid-cols-1 gap-6 max-w-md mx-auto mt-8">
                {features.map((f, i) => (
                    <Reveal key={i} delay={i * 100} className="w-full">
                        <div className="flex items-start gap-4 p-4 rounded-xl transition-all hover-lift"
                            style={{ background: 'rgba(12,12,32,0.75)', backdropFilter: 'blur(20px) saturate(160%)', border: '1px solid rgba(255,255,255,0.08)' }}>
...
export const LoginPage: React.FC<LoginPageProps> = ({ users, onLogin, onSignup }) => {
    // Store current path so SmartCTA can show "Continue exploring X"
    useEffect(() => {
                ...
                <div className="w-full max-w-md mx-auto lg:mx-0">
                    ...
                    <div className="rounded-2xl p-8" style={{
                        ...
                        <form onSubmit={handleLogin} className="space-y-5">
                            ...
                            {/* Login Button (Smart CTA) */}
                            <SmartCTA isLoading={isLoading} />
                       </form>
