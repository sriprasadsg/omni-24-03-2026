...
@router.post("")
async def create_program(payload: dict = Body(...), current_user: TokenData = Depends(rbac_service.has_permission("manage:settings"))):
    db = get_database()
    if not payload.get("name"):
        raise HTTPException(status_code=400, detail="name required")
    doc = await svc.create_program(db, get_tenant_id(), payload)
    return {"program": doc}
...
async def update_program_controls(program_id: str, payload: dict = Body(...), current_user: TokenData = Depends(rbac_service.has_permission("manage:settings"))):
    db = get_database()
    doc = await svc.update_controls(db, program_id, get_tenant_id(), payload.get("add", []), payload.get("remove", []))
    if not doc:
        raise HTTPException(status_code=404, detail="Program not found")
    return {"program": doc}
