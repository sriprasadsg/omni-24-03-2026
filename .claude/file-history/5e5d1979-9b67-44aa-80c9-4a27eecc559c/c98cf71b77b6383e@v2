...
export const PrivacyLegalDashboard: React.FC = () => {
  ...
  const submitTia = async () => { try { await (await authFetch('/api/privacy/tia', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(tiaForm) })).json(); showToast('TIA created', 'success'); setShowTiaForm(false); setTiaForm({}); fetchData(); } catch { showToast('Failed', 'error'); } };
  const submitLia = async () => { try { await (await authFetch('/api/privacy/lia', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(liaForm) })).json(); showToast('LIA created', 'success'); setShowLiaForm(false); setLiaForm({}); fetchData(); } catch { showToast('Failed', 'error'); } };
  const submitNotice = async () => { try { await (await authFetch('/api/privacy/notices', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(noticeForm) })).json(); showToast('Notice created', 'success'); setShowNoticeForm(false); setNoticeForm({}); fetchData(); } catch { showToast('Failed', 'error'); } };
  const submitContract = async () => { try { await (await authFetch('/api/privacy/contracts', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(contractForm) })).json(); showToast('Contract created', 'success'); setShowContractForm(false); setContractForm({}); fetchData(); } catch { showToast('Failed', 'error'); } };
