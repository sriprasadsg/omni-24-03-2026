...
import { SoftwareDeploymentHistoryTab } from './SoftwareDeploymentHistoryTab';
...
export const SoftwareDeployment: React.FC = () => {
    ...
    const fetchDeployHistory = async (signal?: AbortSignal) => {
        try {
            const res = await authFetch('/api/software/tasks', { signal });
            if (res.ok) {
                const data = await res.json();
                setDeployHistory(Array.isArray(data) ? data : []);
            }
        } catch (e) { if ((e as any)?.name !== 'AbortError') console.error('Failed to load deploy history', e); }
    };

    const fetchRepoFiles = async (signal?: AbortSignal) => {
        try {
            const res = await authFetch('/api/software/repository', { signal });
            if (res.ok) setRepoFiles(await res.json());
        } catch (e) { if ((e as any)?.name !== 'AbortError') console.error('Failed to load repo', e); }
    };
        ...
        reader.onload = (event) => {
            try {
                ...
                if (text.includes('\0') || text.startsWith('MZ')) {
                    showToast('Error: Binary files are not supported. Upload a plain text package list.', 'error');
                    return;
                }
                if (text.trim().startsWith('{') || text.trim().startsWith('[')) {
                    const data = JSON.parse(text);
                    if (Array.isArray(data)) showToast('Bulk upload parsed ' + data.length + ' items', 'info');
                } else {
                    const lines = text.split('\n').filter(l => l.trim());
                    if (lines.length > 0) {
                        setPackageId(lines[0]);
                        showToast(`Loaded ${lines.length} packages. Populating first: ${lines[0]}`, 'info');
                    }
                }
