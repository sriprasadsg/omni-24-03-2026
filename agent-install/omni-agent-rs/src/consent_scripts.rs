...
//!
//! Holds the two script constants that `consent_ui.rs` spawns into the
//! interactive session. They are extracted into their own file purely for the
//! 500-line source cap (CLAUDE.md) — the Rust public surface, session
//! bookkeeping, spawn path, and tests all live in `consent_ui.rs`. Nothing
...
# word so the Rust poller never hangs.
$form.Add_FormClosed({
    $timer.Stop()
...
$name.Font           = New-Object System.Drawing.Font("Segoe UI", 10)
